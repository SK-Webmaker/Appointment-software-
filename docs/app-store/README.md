...
| 6 — Build, in shippable slices | [`06-phase-6-build-log.md`](06-phase-6-build-log.md) | Slices 1–6 done (harness, shard, move tool, platform, connectors, the app) |
| 7 — Launch | [`07-phase-7-launch.md`](07-phase-7-launch.md) | **Written — listing, screenshots, privacy answers, review notes and the ten-step order. Waiting on the A$149 Apple enrolment** |
...
| 2026-09-05 | Slice 5 done: the connectors. Email set up for the business inside their own Resend account, with DNS written to Cloudflare, a single domain-wide DMARC, real verification before anything is called done, and a domain-scoped sending key; an own-domain salon gets the records to add and is not marked done until they exist. Texts on the salon’s own existing mobile via ClickSend Own Numbers, key checked live when pasted. Payments optional. A setup checklist computed from what is true, which disappears when it is finished. The 14-day no-reason refund button, automatic inside the window and a task after it. 18 suites / 138 checks / 33 mutations caught — three survived the first run (DNS unchecked, second DMARC, unverified domain accepted) and three tests were written for them. v1.56.0 on this branch only. | `06-…` |
| 2026-09-05 | Slice 6 done: the app. Push to the owner’s phone straight to APNs over node:http2, provider token signed ES256 in JOSE form and cached fifty minutes; credentials on the process, never a salon’s settings, because one app serves every salon. Devices upserted per token and retired on Apple’s 410. A customer booking wakes the phone, collapsed per appointment. Universal links narrowed to /book and /r/. Account deletion inside the app: password plus the typed business name, salon shut immediately, files after seven days. Checklist now detects a signed-in phone. `ios/` is a SwiftUI shell around the same workspace, built on GitHub’s macOS runners — **D14 proven, not assumed: BUILD SUCCEEDED on Xcode 26.6 with no Apple account and no Mac.** 19 files / 153 checks / 41 mutations caught. Found a real bug: the booking push read a column that does not exist and failed silently. v1.57.0 on this branch only. | `06-…`, `ios/README.md` |
| 2026-09-06 | iOS CI falsified: a deliberately broken Lock.swift pushed to a throwaway branch made the macOS build fail — `** BUILD FAILED **`, exit 65, four compiler errors on the exact line. The workflow genuinely compiles and genuinely fails. The throwaway branch `claude/ios-ci-falsify` could not be deleted from this session (the git proxy refuses ref deletions) and needs one click on GitHub. | `06-…` |
| 2026-09-06 | Phase 7 written: the full App Store listing (name, subtitle, description, keywords, category, age rating), five 1320×2868 screenshots generated from the running product by `tools/appstore/`, the privacy questionnaire answered field by field, review notes that pre-empt the 3.1.3(f) question, what it costs (A$149/year and nothing else — public repo, so macOS runners are free), and the ten-step order ending with Phase 5 deliberately after submission. A test now guards the promise that the app quotes no price (42 mutations). | `07-…` |
